...
# Semaphore to respect Pexels 200 req/hr rate limit across concurrent tasks
_pexels_semaphore = asyncio.Semaphore(PEXELS_MAX_CONCURRENCY)
...
def _build_query_variants(
    ...
    llm_q = semantic_llm_query.strip() if semantic_llm_query else ""

    if not base and not llm_q:
        return []

    # If base is empty but LLM query exists, use it as the base so downstream
    # enrichment still has something to work with.
    if not base:
        base = llm_q
    ...
    if SEMANTIC_LLM_QUERY_ENABLED and llm_q:
        if llm_q not in variants:
