...
from app.pipeline.nodes.validation.emoji import assign_emojis_to_beats, lookup_emoji
...
def _align_beats_with_timestamps(
    ...
    Strategy: walk through narration words sequentially, for each LLM beat
    consume words until the running text reasonably matches beat['text'].
    Tolerance: small substring overlap, since LLM may slightly normalize
    punctuation.
    """
    if not word_timestamps:
        # No timestamps available: distribute evenly across scene duration
    ...
    for beat in llm_beats:
        ...
        # Greedily consume words until we've covered target's word count
        # or until consumed text contains target as substring
        ...
        for _ in range(target_word_count):
            ...
            # Early exit if consumed text already covers target meaningfully
    ...
    # Fallback: if alignment dropped beats (cursor stalled), distribute remaining
    if not aligned:
        return _distribute_evenly(llm_beats, scene_start, scene_end)

    # Patch any gaps: ensure last beat ends at scene_end
...
def _normalize_text(t: str) -> str:
    """Normalize text for fuzzy comparison: lowercase, collapse spaces, strip punct."""
...
def _sanitize_emoji(value: Any, fallback: str = "✨") -> str:
    """Return a safe emoji glyph, else fallback.

    LLMs sometimes return plain text, invalid codepoints, or long strings.
    We keep only the first emoji-like glyph (+ optional variation selector).
    """
    raw = str(value or "").strip()
    if not raw:
        return fallback

    match = _EMOJI_CHAR.search(raw)
    if not match:
        return fallback

    idx = match.start()
    glyph = match.group(0)
    if idx + 1 < len(raw) and raw[idx + 1] in {"\ufe0e", "\ufe0f"}:
        glyph += raw[idx + 1]
    return glyph
...
def _split_beats_by_word_gaps(
    ...
    if not word_timestamps:
        # No timestamps — just split by punctuation
...
def _merge_short_beats(
    ...
    """Merge any beat shorter than `min_ms` with its right neighbor (or left if last)."""
    if not beats:
        return beats
    ...
    while i < len(beats):
        ...
        # If short and there is a next beat, merge forward
        if duration < min_ms and i + 1 < len(beats):
            ...
            i += 1
        # If still too short and we have a previous merged, merge with previous
        if (
...
def _reduce_to_max(
    ...
    while len(out) > max_count:
        # Find adjacent pair with smallest combined duration
        best_i = 0
        best_dur = float("inf")
...
def _build_single_beat(text: str, start_ms: float, end_ms: float) -> dict[str, Any]:
    """Build a single beat covering the entire narration with rule-based emoji."""
    return {
        "text": text,
        "emoji": lookup_emoji(text),
        "start_ms": start_ms,
        "end_ms": end_ms,
    }
