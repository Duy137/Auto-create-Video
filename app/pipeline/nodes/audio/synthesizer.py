...
from app.state import WordTimestamp, TokenUsage, calc_cost
...
class OpenAITTSEngine(TTSEngine):
    ...
    async def synthesize(
        ...
        try:
            ...
            # Estimate tokens: ~86 output tokens per second of audio
            estimated_output_tokens = int(duration_ms / 1000 * 86)
            token_usage = TokenUsage(
                model="gpt-4o-mini-tts",
                step="tts",
                input_tokens=0,
                output_tokens=estimated_output_tokens,
                cost_usd=calc_cost("gpt-4o-mini-tts", 0, estimated_output_tokens)
            )
...
class VbeeTTSEngine(TTSEngine):
    ...
    POLL_TIMEOUT = 180   # max wait seconds (long text needs more time)
    ...
    async def synthesize(
        ...
        try:
            # ── Step 1: Create speech request (with retry) ──
            create_data = None
            _TRANSIENT_ERRORS = (httpx.ConnectTimeout, httpx.ReadTimeout, httpx.ConnectError, httpx.RemoteProtocolError)
            ...
            for attempt in range(3):
                try:
                    ...
                    logger.warning(
                        "Vbee TTS: create request attempt {}/3 failed: {} — {}",
                        attempt + 1, type(net_err).__name__, str(net_err)[:200]
                    )
                    if attempt < 2:
                        await asyncio.sleep(2 * (attempt + 1))
                    else:
                        raise TTSError(f"Vbee TTS: failed to create request after 3 attempts: {type(net_err).__name__} - {net_err}") from net_err
                except httpx.HTTPStatusError as http_err:
                    logger.error("Vbee TTS HTTP error: {} - {}", http_err.response.status_code, http_err.response.text)
                    raise TTSError(f"Vbee TTS API Error {http_err.response.status_code}: {http_err.response.text}") from http_err

            if not create_data:
                raise TTSError("Vbee TTS: Failed to get response data from API")
            ...
            # ── Step 3: Download audio (with retry for transient network errors) ──
            audio_bytes = None
            ...
            logger.exception("Vbee TTS unhandled exception")
            raise TTSError(f"Vbee TTS failed: {error_msg}") from e
